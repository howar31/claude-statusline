...
- **Optional payload fields render gated.** Fields that may be absent (`pr.*`, `worktree.*`, `thinking.enabled`, `context_window.context_window_size`, `cost.total_api_duration_ms`, `session_name`) are shown only when present / non-zero, so a line never emits a stray separator or placeholder. Keep new field additions gated the same way. See `SPEC.md` → Gated fields.
